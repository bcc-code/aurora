<template>
    <li class="mb-2" @click.stop="goToEvent">
        <div class="cursor-pointer list-item" :class="[isCurrent ? 'bg-green-500' : 'bg-background-2']">
            ...
            <div v-if="$can('update', event)" class="overlay">
                <button v-if="isCurrent" class="btn btn-red" @click.stop="end">{{$t('actions.end')}}</button>
                <button v-else class="btn btn-green" @click.stop="start">{{$t('actions.start')}}</button>
...
export default {
    ...
    computed: {
        ...
        text() {
            return (this.searchQuery && this.searchQuery.trim()) 
                ? this.event.name.replace(new RegExp(`(${this.searchQuery})`, "gi"), "<span class='bg-blue-500 rounded-sm'>\$1</span>") 
                : this.event.name;
        },
        details() {
            return (this.searchQuery && this.searchQuery.trim() && this.event.details) 
                ? this.event.details.replace(new RegExp(`(${this.searchQuery})`, "gi"), "<span class='bg-blue-500 rounded-sm'>\$1</span>") 
                : this.event.details;
        },
        isCurrent() {
    ...
    methods: {
        ...mapActions('events', ['removeEventRef', 'startEventRef', 'endEventRef']),
        async remove(){
        ...
        async start(){
            await this.startEventRef(this.event.id);
        },
        async end(){
            await this.endEventRef(this.event.id);
        },
        goToEvent() {
