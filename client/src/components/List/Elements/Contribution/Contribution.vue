<template>
    <li class="mb-2 xl:w-1/4 lg:w-1/3 md:w-1/2 w-full px-4">
        <div class="transition duration-500 p-0 rounded relative" :class="[selected ? 'gradient-secondary' : 'bg-mirage']" @click="toggleSelect">
            <div class="px-4 py-3">
                <div class="font-semibold text-label-1 tracking-wide mb-2" v-html="highlight(displayName)"></div>
                <div v-if="churchAndCountry" class="text-slate-gray text-sm mb-2" v-html="highlight(churchAndCountry)"></div>
                <div class="text-slate-gray text-sm mb-2">{{date(entry.date)}}</div>
                <span class="text-label-2 text-base" v-html="highlight(entry.text)"></span>
