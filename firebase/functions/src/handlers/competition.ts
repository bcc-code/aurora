...
export async function submitCompetitionEntry(db : firestore.Firestore, req : Request, res : Response) {
  ...
  const personId = req.body.personId ? req.body.personId : loggedInUserPersonId;
  ...
  try {
    const competitionModel = new CompetitionModel(db, firebaseAdmin.firestore.FieldValue.increment, req.query.competitionId);
    const result = await competitionModel.updateEntry(personId, req.body.distance, req.body.overrideMaxDistance || 0);
