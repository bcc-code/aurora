...
export async function addEntry(
    ...
    if (req.query.bukGameId !== 'fktb2101') {
        return res
            .status(400)
            .send({ message: 'The bukGameId is invalid' })
            .end()
    }
    if (!personId) {
        return res
        ...
        if (timestamp > Date.now()) {
            ...
            const timeRemaining =
                minRemaining === 0
                    ...
                    : `${minRemaining} minute(s)`
            return res
                .status(400)
                .send({
                    message: `You have been banned. Please retry in ${timeRemaining}`,
                })
                .end()
        } else {
    ...
    if (!(req.body.score >= 0)) {
        return res
            .status(400)
            .send({
                message: `You won't reach the leaderboard with a negative score buddy`,
            })
            .end()
    }
    ...
    try {
        ...
        const result = await bukGameModel.updateEntry(
            personId,
            req.body.game,
            req.body.score
        )
