...
export class UserModel {
    ...
    async updateInternal(
        ...
        if (
            ...
            (update.genderId !== null &&
                update.genderId !== userData.genderId) ||
            ...
            (update.churchId !== null &&
                update.churchId !== userData.churchId) ||
            (update.churchName !== null &&
                update.churchName !== userData.churchName) ||
    ...
    async createInternal(created: IUser): Promise<DocumentSnapshot> {
        log.info(
            `Creating new user, ID: ${created.personId}, displayName: ${
                created.displayName ?? ''
            }`
        )
        const localBatch = this.db.batch()
