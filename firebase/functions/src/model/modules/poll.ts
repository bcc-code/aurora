...
export class PollModule extends Module {
  ...
  response(personId: string, questionId: string): firestore.DocumentReference {
  ...
  async setPollResponse(
    ...
    previousAnswers.forEach((answerId: string) => {
      ...
      bucketNames.forEach((bucketName: string) => {
        ...
        batch.set(this.qaShard(questionId, answerId, shardId), updatedDoc, {
          merge: true,
        });
      });
    });
  ...
  async pickRandomWinner(questionId: string): Promise<boolean> {
    ...
    switch (question.type) {
      ...
      case "slider":
        correctResponsesList = await this.responses
          .where("question", "==", questionId)
          .where("selected", "array-contains", question.slider.correct || 0)
          .get();
        ...
        candidates = (
          await this.responses.where("question", "==", questionId).get()
  ...
  async updatePollStats(currentQuestionId: string): Promise<{}> {
    ...
    for (let i = 0; i < shards.length; i++) {
      ...
      if (questionId === currentQuestionId) {
        ...
        const answerWrapper: { [i: string]: Record<string, unknown> } = {};
              ...
              if (userBucketValue != null && userBucketValue != "") {
                response.selected.forEach((selectedAnswer: string) => {
                  ...
                  _set(
                    result,
                    path,
                    !isNumber(currentBucketValue) ? 1 : currentBucketValue + 1
                  );
                });
              }
            });
